...
AI>> 경계처리 코드에서 경계 밖으로 나가지 못하게 하는 코드에 반지름을 증가시킴.

<이 상태에서 경계에 부딪힐 시 크기가 멈추지 않고 계속 커지는 문제가 있음>

AI<< 도형이 경계에 한번 부딪혔을 때만 크기가 커지게 해줘

was_hit 변수를 추가해 벽에 부딪힌 첫 순간만 크기가 커지게 만듬.
